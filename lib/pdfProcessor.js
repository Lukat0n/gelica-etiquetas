...
export async function generatePDF(arrayBuffer, labels, comboOrder = null, aliases = {}, stripSize = 'normal') {
  const buffer = arrayBuffer.slice(0);
  ...
  for (const label of sortedLabels) {
    ...
    // Agrandar la página agregando pie para la tira
    const extraH = STRIP_H + FOOTER_PAD * 2;
    copied.setSize(width, height + extraH);
    // Mover todo el contenido existente hacia arriba para dejar espacio abajo
    copied.translateContent(0, extraH);

    newDoc.addPage(copied);
    const pages = newDoc.getPages();
    const page = pages[pages.length - 1];

    // Dibujar la tira en el pie nuevo (Y=0 es abajo)
    await drawStripAtY(page, label.combo, FOOTER_PAD, STRIP_H, aliases);
  }
