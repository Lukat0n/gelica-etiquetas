...
function extractOrderNumber(items) {
  // Patrones ordenados por especificidad (el primero que matchee gana)
  const patterns = [
    // #12345 (E-Pick, común)
    { regex: /^#(\d{4,6})$/, extract: m => m[1] },
    // "Pedido #12345" o "Orden #12345"
    { regex: /(?:pedido|orden|order)\s*#?\s*(\d{4,6})/i, extract: m => m[1] },
    // Número de orden en posición Cabify (coord específica)
    { regex: /^\d{4,6}$/, extract: m => m[0], filter: item => item.x > 85 && item.x < 100 && item.y > 295 && item.y < 310 },
    // "Nro: 12345" o "N°: 12345"
    { regex: /(?:nro|n°|número|numero|ref)\s*[:.]?\s*(\d{4,6})/i, extract: m => m[1] },
    // Número suelto con # adelante en cualquier posición
    { regex: /#(\d{4,6})/, extract: m => m[1] },
  ];

  for (const { regex, extract, filter } of patterns) {
  ...
  // Último recurso: buscar cualquier número de 4-6 dígitos suelto (no teléfonos, no CPs)
  // Solo si hay pocos candidatos para evitar falsos positivos
  const candidates = [];
  for (const item of items) {
    const txt = item.str.trim();
    if (/^\d{4,6}$/.test(txt)) candidates.push(txt);
  }
...
async function parseLabels(pdfDoc, products, credentials) {
  ...
  // Fetchear productos desde TiendaNube API (con cache)
  const cache = new Map();
...
export async function processPDF(arrayBuffer, sortByProduct = false, products = DEFAULT_PRODUCTS, credentials = null, stripPct = 0.95, aliases = {}) {
  ...
  let labels = await parseLabels(pdfDoc, products, credentials);
