...
function mapTiendaNubeName(rawName) {
  const lower = rawName.toLowerCase();
  for (const { match, internal } of TIENDANUBE_NAME_MAP) {
    if (lower.includes(match)) return internal;
  }
  return rawName; // producto desconocido → se muestra en gris
}
...
async function parseCabifyLabels(pdfDoc, products) {
  const labels = [];
  for (let p = 1; p <= pdfDoc.numPages; p++) {
    ...
    const combo = peso > 0 ? findCombo(peso, products) : null;
...
async function fetchOrderFromTiendaNube(ordenNumber, credentials) {
  ...
  for (const item of order.products) {
    const name = mapTiendaNubeName(item.name || '');
...
async function drawStrip(page, combo) {
  const { width } = page.getSize();
  const stripH = 38, stripY = 8, padX = 8;
  page.drawRectangle({
...
export async function processPDF(arrayBuffer, sortByProduct = false, products = DEFAULT_PRODUCTS, credentials = null) {
  ...
  let labels = format === 'epick'
    ...
    : await parseCabifyLabels(pdfDoc, products);
  ...
  for (const label of labels) {
    ...
    await drawStrip(pages[pages.length - 1], label.combo);
  }
