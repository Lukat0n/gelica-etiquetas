...
function extractOrderNumber(items) {
  ...
  const itemPatterns = [
    ...
    // "Orden #12345" dentro de un solo item
...
export async function processPDF(arrayBuffer, sortByProduct = false, products = DEFAULT_PRODUCTS, credentials = null, stripPct = 0.95, aliases = {}, sortOrder = null) {
  ...
  if (sortByProduct) {
    labels = [...labels].sort((a, b) => {
      const ka = comboSortKey(a.combo, sortOrder);
      const kb = comboSortKey(b.combo, sortOrder);
