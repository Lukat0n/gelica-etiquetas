...
async function parseEpickLabels(pdfDoc, products) {
  const labels = [];
  for (let p = 1; p <= pdfDoc.numPages; p++) {
    const page = await pdfDoc.getPage(p);
    const items = await getPageItems(page);
...
export async function processPDF(arrayBuffer, sortByProduct = false, products = DEFAULT_PRODUCTS) {
  ...
  let labels = format === 'epick'
    ? await parseEpickLabels(pdfDoc, products)
