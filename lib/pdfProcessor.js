...
async function drawStrip(page, combo, position = 'abajo') {
  const { width, height } = page.getSize();
  const stripH = 38, padX = 8;
  let stripY = 8; // abajo
  if (position === 'arriba') stripY = height - stripH - 8;
  else if (position === 'centro') stripY = (height - stripH) / 2;
  page.drawRectangle({
...
export async function processPDF(arrayBuffer, sortByProduct = false, products = DEFAULT_PRODUCTS, credentials = null, stripPosition = 'abajo') {
  ...
  for (const label of labels) {
    ...
    await drawStrip(pages[pages.length - 1], label.combo, stripPosition);
  }
